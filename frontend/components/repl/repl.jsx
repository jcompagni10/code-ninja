      ...
      return (
        <section className="repl_container">
          <section className="left-pane">
            <header className="task-header header-left">description</header>
            <section className="task-description">
              <p className="task-example">{task.description}</p>
              <h4>Example</h4>
              <p
                className="example"
                dangerouslySetInnerHTML={{ __html: task.example }}
              />
              <h4>Input/Output</h4>
              <ul className="io-list">
                <li>
                  <h4>[time limit] {task.time_limit}ms</h4>
                </li>
                {task.inputs.map(input => (
                  <li key={input.id}>
                    ...
                    <span className="input-constraints-label">
                      ...
                      <span
                        className="code"
                        dangerouslySetInnerHTML={{ __html: input.constraints }}
                      />
                    </span>
                  </li>
                ))}
                <li>
                  <h4>[output] {task.output_type}</h4>
          ...
          <section className="right-pane">
            <header className="task-header header-right bottom-bar">
              <div className="file-name pull-left">code.rb</div>
              <div className="reset pull-right" onClick={() => this.reset()}>
